...
public:
  ...
  string listItem;   //group name like family
  ...
  vector<string> &nextViableStates() {       //return next viable state
    if (state == states[0]) {
      return nextGroups;
    } else {
      return nextGroup;
  ...
  bool doAction(string action) {
    ...
    } else if (state == states[1]) {     // states are only 2... splitwise and group
      if (action == format(actionGroup[0])) {
        Group &grp = groups.refGroup(listItem);              //returns reference of the group
        grp.addExpense();
        return true;
