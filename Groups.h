...
class Groups {
  ...
  void sortList() { sort(groups.begin(), groups.end(), comparator); }    //alphabetically sort all groups
...
public:
  ...
  string addGroup() {
    ...
    if (name == "group") {           //name of the group should not be kept 'group'
      throw 1;
    }
  ...
  void viewGroups() {
    sortList();               //output name of the groups
